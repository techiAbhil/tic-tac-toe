	...
	const drawXO = (boxID) => {
		...
		lineArray.forEach((line) => {
			if (line.every((columnIndex) => updatedBoxState[columnIndex] === 'X')) {
				...
				line.every((columnIndex) => updatedBoxState[columnIndex] === 'O')
			) {
		...
		<main>
			<section className="box-container">
				{boxState.map((_, index) => {
					const shouldHighlight = winningLine.some(
					...
					return (
						<div
							key={index}
							id={`box-${index}`}
							className={`box ${shouldHighlight ? 'highlight-div' : ''}`}
							onClick={() => drawXO(index)}
						>
							{boxState[index] ?? ''}
						</div>
					);
